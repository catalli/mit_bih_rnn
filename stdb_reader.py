...
output_path = ''.join([script_path, '/stdb_delight.pkl'])
print("Reading data from ",dl_path)
wfdb.dldatabase("stdb", dl_path)
file_names = []
...
class_map = {'N':'N','L':'N','R':'N','A':'S','a':'S','J':'S','S':'S','V':'V','e':'N','j':'N','E':'V','/':'N','f':'N','!':'V'}
...
type_quotas = [1000,6000,6000,6000,6000,6000,6000,6000,6000,6000,6000,6000,6000,6000,6000]

test_quotas = [1500,1500,1500,1500,1500,1500,1500,1500,15000,1500,1500,1500,1500,1500,1500]
for f in file_names:
	...
	for a in range(len(annotation.annsamp)):
		if annotation.anntype[a] in beat_classes:
			beat_anns.append([annotation.annsamp[a], annotation.anntype[a]])
                        print([annotation.annsamp[a], annotation.anntype[a]])
...
data = [np.zeros((no_seqs,max_len,1), dtype=np.float32), np.zeros((no_seqs, len(aami_classes)), dtype=np.float32), np.zeros((no_seqs),dtype=np.int32)]
