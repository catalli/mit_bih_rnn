...
if len(sys.argv) > 1:
	max_b_length = int(sys.argv[1])
        data_save_path = ''.join([script_path, "/ltafdb_projected_", sys.argv[1], ".pkl"])
        reuse_dict = False
else:
	data_save_path = ''.join([script_path, "/ltafdb_projected.pkl"])

_dict_path = ''.join([script_path, '/ltafdb_delight_dict.pkl'])
